...
class DAO():
    ...
    def getAllEdgesV2(idMapAirports):
        ...
        cursor = conn.cursor(dictionary=True)
        query = """select t1.ORIGIN_AIRPORT_ID, t1.DESTINATION_AIRPORT_ID, coalesce(t1.n, 0) + coalesce(t2.n,0)
        ...
        for row in cursor:
            # result.append((idMapAirports[row['ORIGIN_AIRPORT_ID']], idMapAirports[row['DESTINATION_AIRPORT_ID']], row['n']))
            result.append(
                Arco(idMapAirports[row['ORIGIN_AIRPORT_ID']], idMapAirports[row['DESTINATION_AIRPORT_ID']], row['n']))
